#!/usr/bin/env bash

###############################################################################
# Experiment Script Template (Reference Only)
#
# Philosophy
# 1) Intent over convenience:
#    - No hidden defaults for important experiment inputs.
#    - Every run must declare key variables explicitly.
#
# 2) Simple runner, rich evidence:
#    - Keep this script easy to read.
#    - Capture raw artifacts (proc/disk/iostat/time), not over-processed metrics.
#
# 3) Reproducibility first:
#    - Save selected options near output.
#    - Save the exact command used for execution.
#
# 4) Separation of concerns:
#    - This script runs experiments and captures raw data.
#    - Parsing/plotting/aggregation belongs to separate scripts.
###############################################################################

: <<'EXAMPLE'
TARGET_DB_GB=25 KV_SIZE=91 CACHE_SIZE_GB=8 \
DB_BENCH=/path/to/db_bench \
DB_ROOT=/work/db USE_NUMACTL=0 NUMA_NODE=0 \
bash your_experiment.sh
EXAMPLE

set -euo pipefail

usage() {
  cat <<'USAGE'
Usage:
  TARGET_DB_GB=<GB> \
  KV_SIZE=<bytes> \
  CACHE_SIZE_GB=<GB> \
  DB_BENCH=<path> \
  DB_ROOT=<path> \
  USE_NUMACTL=<0|1> \
  NUMA_NODE=<node> \
  bash your_experiment.sh
USAGE
}

require_env() {
  local name="$1"
  [[ -n "${!name:-}" ]] || {
    echo "[ERROR] Missing required env: ${name}" >&2
    usage >&2
    exit 1
  }
}

# Required inputs: no defaults for key knobs.
for name in TARGET_DB_GB KV_SIZE CACHE_SIZE_GB DB_BENCH DB_ROOT USE_NUMACTL NUMA_NODE; do
  require_env "$name"
done

# Optional validation examples.
[[ "${KV_SIZE}" == "91" ]] || { echo "[ERROR] KV_SIZE must be 91" >&2; exit 1; }
[[ "${USE_NUMACTL}" == "0" || "${USE_NUMACTL}" == "1" ]] || { echo "[ERROR] USE_NUMACTL must be 0 or 1" >&2; exit 1; }
[[ "${USE_NUMACTL}" == "0" ]] || command -v numactl >/dev/null 2>&1 || { echo "[ERROR] numactl not found" >&2; exit 1; }

# Local fixed policy (acceptable constants).
LOG_ROOT="log_loads"
MAX_OPEN_FILES_LIMIT=1048576
RUN_TS="$(date '+%y%m%d_%H%M')"
RUN_DIR="${LOG_ROOT}/run_${RUN_TS}_${TARGET_DB_GB}gb"
OUT_FILE="${RUN_DIR}/default.out"
OPTIONS_FILE="${RUN_DIR}/default.options"
RAW_DIR="${RUN_DIR}/raw"
CMD_FILE="${RAW_DIR}/run_cmd.sh"

# Domain variables (replace for each experiment).
DB_DIR="${DB_ROOT%/}/${TARGET_DB_GB}gb_experiment"
KEY_SIZE=48
VALUE_SIZE=43
MIB=$((1024 * 1024))
CACHE_BYTES=$((1024 * 1024 * 1024 * CACHE_SIZE_GB))
DB_SIZE_BYTES=$((TARGET_DB_GB * 1024 * 1024 * 1024))
NKEYS=$((DB_SIZE_BYTES / KV_SIZE))

# Safety checks.
[[ ! -d "${DB_DIR}" ]] || { echo "[ERROR] DB already exists: ${DB_DIR}" >&2; exit 1; }
mkdir -p "${RUN_DIR}" "${RAW_DIR}" "${DB_DIR}"
ulimit -n "${MAX_OPEN_FILES_LIMIT}"

# Command construction.
cmd_prefix=()
if [[ "${USE_NUMACTL}" == "1" ]]; then
  cmd_prefix=(numactl --membind="${NUMA_NODE}" --cpunodebind="${NUMA_NODE}")
fi

cmd=(
  "${cmd_prefix[@]}"
  "${DB_BENCH}"
  # Replace below with your experiment-specific flags.
  --benchmarks=filluniquerandom,stats,levelstats
  --cache_size="${CACHE_BYTES}"
  --num="${NKEYS}"
  --key_size="${KEY_SIZE}"
  --value_size="${VALUE_SIZE}"
  --db="${DB_DIR}"
)

# Save selected options near output (human-readable + grep-friendly).
{
  echo "run_ts=${RUN_TS}"
  echo "run_dir=${RUN_DIR}"
  echo "db_dir=${DB_DIR}"
  echo "target_db_gb=${TARGET_DB_GB}"
  echo "kv_size=${KV_SIZE}"
  echo "cache_size_gb=${CACHE_SIZE_GB}"
  echo "db_bench=${DB_BENCH}"
  echo "use_numactl=${USE_NUMACTL}"
  echo "numa_node=${NUMA_NODE}"
  echo "max_open_files_limit=${MAX_OPEN_FILES_LIMIT}"
  echo "key_size=${KEY_SIZE}"
  echo "value_size=${VALUE_SIZE}"
  echo "cache_bytes=${CACHE_BYTES}"
  echo "db_size_bytes=${DB_SIZE_BYTES}"
  echo "nkeys=${NKEYS}"
} > "${OPTIONS_FILE}"

# Log command to output and save re-runnable command script.
{
  echo "[RUN_CMD]"
  printf '%q ' "${cmd[@]}"
  echo
} | tee "${OUT_FILE}"

{
  printf '#!/usr/bin/env bash\n'
  printf '%q ' "${cmd[@]}"
  echo
} > "${CMD_FILE}"

# Raw snapshots (start).
start_ts="$(date +%s)"
echo "${start_ts}" > "${RAW_DIR}/start_epoch.txt"
cat /proc/diskstats > "${RAW_DIR}/diskstats.start"
cat /proc/stat > "${RAW_DIR}/procstat.start"
if command -v iostat >/dev/null 2>&1; then
  iostat -dx 1 > "${RAW_DIR}/iostat.log" &
  iostat_pid=$!
else
  iostat_pid=""
  echo "iostat_not_found" > "${RAW_DIR}/iostat.log"
fi

# Run.
"${cmd[@]}" >> "${OUT_FILE}" 2>&1

# Stop background samplers.
if [[ -n "${iostat_pid}" ]]; then
  kill "${iostat_pid}" 2>/dev/null || true
  wait "${iostat_pid}" 2>/dev/null || true
fi

# Raw snapshots (end).
end_ts="$(date +%s)"
elapsed_sec=$((end_ts - start_ts))
echo "${end_ts}" > "${RAW_DIR}/end_epoch.txt"
echo "${elapsed_sec}" > "${RAW_DIR}/elapsed_sec.txt"
cat /proc/diskstats > "${RAW_DIR}/diskstats.end"
cat /proc/stat > "${RAW_DIR}/procstat.end"

# End of template. Post-processing should be done in another script.
