...
DB_DIR="${DB_ROOT%/}/${TARGET_DB_GB}gb_fillseqow"
...
OVERWRITE_KEYS=$((NKEYS * OVERWRITE_PCT / 100))
LOAD_THREADS=1
...
overwrite_cmd=(
  "${common_opts[@]}"
  --benchmarks=overwrite,stats,levelstats
  --report_file="${RUN_DIR}/default.rep"
  --use_existing_db=true
  --writes="${OVERWRITE_KEYS}"
)

{
  ...
  echo "overwrite_pct=${OVERWRITE_PCT}"
  echo "overwrite_keys=${OVERWRITE_KEYS}"
  ...
  echo "benchmarks=fillseq,stats,levelstats -> overwrite,stats,levelstats"
  echo "overwrite_pct=${OVERWRITE_PCT}"
  echo "overwrite_keys=${OVERWRITE_KEYS}"
} > "${OPTIONS_FILE}"
...
{
  echo "[RUN_CMD]"
  printf '%q ' "${fillseq_cmd[@]}"
  echo
  printf '%q ' "${overwrite_cmd[@]}"
  echo
} | tee "${OUT_FILE}"
...
"${fillseq_cmd[@]}" >> "${OUT_FILE}" 2>&1
"${overwrite_cmd[@]}" >> "${OUT_FILE}" 2>&1
