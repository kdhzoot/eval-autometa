...
DEFAULT_OUTPUT = (SCRIPT_DIR / "perf_context_hit_by_level.png").resolve()
DEFAULT_OUTPUT_CSV = (SCRIPT_DIR / "perf_context_hit_by_level_data.csv").resolve()
...
def plot_hit_level(
    ...
    plt.xlabel("LSM Level", fontsize=30)
    plt.ylabel(f"{metric_label} (%)", fontsize=30)
    plt.title(f"{metric_label} by LSM Level", fontsize=36)
    plt.xticks(sorted({lvl for key in keys for lvl in grouped[key].keys()}), fontsize=20)
    plt.yticks(fontsize=20)
    plt.grid(True, alpha=0.25)
    plt.ylim(0.0, 103.0)
    plt.legend(loc="best", fontsize=18, title="DB Size | Filter | Cache %", title_fontsize=20)
    plt.tight_layout()
...
def main() -> None:
    ...
    all_grouped = _read_rows(Path(args.input), args.metric, db_size_filter="")
    if not all_grouped:
